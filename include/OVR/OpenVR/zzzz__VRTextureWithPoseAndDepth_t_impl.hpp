#pragma once
// IWYU pragma private; include "OVR/OpenVR/VRTextureWithPoseAndDepth_t.hpp"
#include "OVR/OpenVR/zzzz__VRTextureDepthInfo_t_impl.hpp"
#include "OVR/OpenVR/zzzz__VRTextureWithPoseAndDepth_t_def.hpp"
#include "OVR/OpenVR/zzzz__VRTextureDepthInfo_t_def.hpp"
// Ctor Parameters [CppParam { name: "depth", ty: "::OVR::OpenVR::VRTextureDepthInfo_t", modifiers: "", def_value: Some("{}") }]
constexpr ::OVR::OpenVR::VRTextureWithPoseAndDepth_t::VRTextureWithPoseAndDepth_t(::OVR::OpenVR::VRTextureDepthInfo_t  depth) noexcept  {
this->depth = depth;
}
// Ctor Parameters []
constexpr ::OVR::OpenVR::VRTextureWithPoseAndDepth_t::VRTextureWithPoseAndDepth_t()   {
}
